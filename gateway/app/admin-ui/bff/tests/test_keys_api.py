...
class FakePricingStore:
    def __init__(self):
        self.doc = {
            ...
            "doc_type": "pricing",
            "region": "eastus2",
            "models": {"gpt-5.4": {"prompt": 0.0025, "completion": 0.015}},
            "sourceModels": {"gpt-5.4": {"prompt": 0.0025, "completion": 0.015}},
            "overrides": {},
        }
...
def ctx():
    ...
    deps = AppDeps(settings=_settings(), apim=apim, store=store, spa_dir=None,
                   ...
                   consumerregistry=FakeConsumerRegistry(), pricing=FakePricingStore(),
...
def test_pricing_get_returns_current_doc():
    app, client, _ = _ctx_with_consumerconfig()
    _as(app, ADMIN)
    body = client.get("/api/pricing").json()
    assert body["region"] == "eastus2"
    assert body["models"]["gpt-5.4"]["completion"] == 0.015


def test_pricing_put_saves_manual_overrides():
    app, client, _ = _ctx_with_consumerconfig()
    _as(app, ADMIN)
    r = client.put("/api/pricing", json={"overrides": {"gpt-5.4": {"prompt": 0.003, "completion": 0.02}}})
    assert r.status_code == 200
    body = r.json()
    assert body["models"]["gpt-5.4"] == {"prompt": 0.003, "completion": 0.02}
    assert body["overrides"]["gpt-5.4"] == {"prompt": 0.003, "completion": 0.02}


def test_pricing_preview_uses_worker_fetcher(monkeypatch):
    from bff import models_pricing

    monkeypatch.setattr(models_pricing._worker_pricing, "fetch_model_pricing", lambda region: {
        "gpt-5.4": {"prompt": 0.004, "completion": 0.02},
        "grok-4.3": {"prompt": 0.001, "completion": 0.004},
    })
    app, client, _ = _ctx_with_consumerconfig()
    _as(app, ADMIN)
    r = client.post("/api/pricing/preview", json={"region": "eastus2"})
    assert r.status_code == 200
    body = r.json()
    assert body["models"]["grok-4.3"]["completion"] == 0.004
    assert any(row["model"] == "gpt-5.4" for row in body["diff"])


def test_pricing_refresh_persists_preview_result(monkeypatch):
    from bff import models_pricing
    ...
    app, client, _ = _ctx_with_consumerconfig()
    _as(app, ADMIN)
    r = client.post("/api/pricing/refresh", json={"region": "eastus2", "overrides": {"gpt-5.4": {"prompt": 0.003, "completion": 0.019}}})
    assert r.status_code == 200
    body = r.json()
    assert body["models"]["gpt-5.4"] == {"prompt": 0.003, "completion": 0.019}


def test_pricing_put_rejects_nonpositive_override():
    app, client, _ = _ctx_with_consumerconfig()
    _as(app, ADMIN)
    r = client.put("/api/pricing", json={"overrides": {"gpt-5.4": {"prompt": 0, "completion": 0.02}}})
    assert r.status_code == 400


def test_pricing_endpoints_require_admin():
    app, client, _ = _ctx_with_consumerconfig()
    _as(app, USER)
    assert client.get("/api/pricing").status_code == 403
    assert client.post("/api/pricing/preview", json={}).status_code == 403
    assert client.post("/api/pricing/refresh", json={}).status_code == 403
    assert client.put("/api/pricing", json={"overrides": {}}).status_code == 403
